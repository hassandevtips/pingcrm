<template>
  <div>
    ...
    <Head title="Leads" />
    <h1 class="mb-8 text-3xl font-bold">Leads</h1>
    <div class="flex items-center justify-between mb-6">
      ...
      <Link class="btn-indigo" href="/leads/create">
      <span>Create</span>
    ...
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="lead in leads.data" :key="lead.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/leads/${lead.id}/edit`">
            {{ lead.name }}
            <icon v-if="lead.deleted_at" name="trash" class="shrink-0 ml-2 w-3 h-3 fill-gray-400" />
            </Link>
          </td>
          <td class="border-t">
            <Link class="flex items-center px-6 py-4" :href="`/leads/${lead.id}/edit`" tabindex="-1">
            <div v-if="lead.organization">
          ...
          <td class="w-px border-t">
            <Link class="flex items-center px-4" :href="`/leads/${lead.id}/edit`" tabindex="-1">
